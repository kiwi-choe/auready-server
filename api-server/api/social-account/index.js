...
var UserController = require(__appbase_dirname + '/models/user.controller');
...
router.post('/login/:socialapp', (req, res) => {
    ...
    const registerSocialAccount = (name, email, password) => {
        User.findOne({'password': password}, (err, user) => {
            ...
            UserController.create(name, email, password, false, (err, newUser) => {
                if (err) {
                    return res.sendStatus(401);
                }
